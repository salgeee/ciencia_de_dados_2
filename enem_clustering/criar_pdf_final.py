...
print("="*60)
# Carregar dados
df = pd.read_csv('data/processed/escolas_com_clusters.csv')
df_30 = pd.read_csv('data/processed/escolas_com_clusters_30pct.csv')
# Criar documento
doc = SimpleDocTemplate(
    "relatorio/Relatorio_Tecnico_ENEM_Clustering.pdf",
    pagesize=A4,
    rightMargin=2*cm,
    leftMargin=2*cm,
    topMargin=2*cm,
...
# Metadados
meta_text = f"""
<b>Autor:</b> Leonardo Ferreira Salge ENEM<br/>
...
if Path('relatorio/figuras/figura1_analise_geral.png').exists():
    story.append(Paragraph("Figura 1: Análise Geral dos Clusters", style_heading3))
    img = Image('relatorio/figuras/figura1_analise_geral.png', width=16*cm, height=11*cm)
    story.append(img)
story.append(PageBreak())
if Path('relatorio/figuras/figura2_composicao_clusters.png').exists():
    story.append(Paragraph("Figura 2: Composição dos Clusters", style_heading3))
    img2 = Image('relatorio/figuras/figura2_composicao_clusters.png', width=16*cm, height=7*cm)
    story.append(img2)
story.append(PageBreak())
# 5. DISCUSSÃO
...
print("="*60)
print(f"\nArquivo: relatorio/Relatorio_Tecnico_ENEM_Clustering.pdf")
